...
The system SHALL provide a `SABRModel` dataclass with fitted fields `alpha`, `beta`, `rho`, `nu` and context fields `expiry` and `forward` representing the SABR stochastic volatility model. `SABRModel` SHALL conform to the `SmileModel` protocol.

#### Scenario: Create SABRModel
- **WHEN** a user constructs `SABRModel(alpha=0.2, beta=0.5, rho=-0.3, nu=0.4, expiry=1.0, forward=100.0)`
- **THEN** all six fields are stored and accessible as attributes
...
`SABRModel` SHALL validate on construction: `alpha > 0`, `0 <= beta <= 1`, `-1 < rho < 1`, `nu >= 0`, `expiry > 0`, `forward > 0`.

#### Scenario: Negative alpha rejected
- **WHEN** a user constructs `SABRModel(alpha=-0.1, beta=0.5, rho=-0.3, nu=0.4, expiry=1.0, forward=100.0)`
- **THEN** a `ValueError` SHALL be raised with a message containing "alpha"

#### Scenario: Beta out of range rejected
- **WHEN** a user constructs `SABRModel(alpha=0.2, beta=1.5, rho=-0.3, nu=0.4, expiry=1.0, forward=100.0)`
- **THEN** a `ValueError` SHALL be raised with a message containing "beta"

#### Scenario: Rho out of range rejected
- **WHEN** a user constructs `SABRModel(alpha=0.2, beta=0.5, rho=1.0, nu=0.4, expiry=1.0, forward=100.0)`
- **THEN** a `ValueError` SHALL be raised with a message containing "rho"

#### Scenario: Nu zero is valid
- **WHEN** a user constructs `SABRModel(alpha=0.2, beta=0.5, rho=-0.3, nu=0.0, expiry=1.0, forward=100.0)`
...
`SABRModel` SHALL serialise only its fitted parameters `(alpha, beta, rho, nu)` via `to_array()` and `from_array()`. The context fields `expiry` and `forward` SHALL NOT be included in the array.
...
- **WHEN** `SABRModel.from_array(model.to_array())` is called
- **THEN** the resulting `SABRModel` SHALL have the same fitted parameter values
...
`SABRModel` SHALL implement `evaluate(x)` using Hagan et al. (2002) lognormal implied volatility approximation, where `x` is log-moneyness and the result is implied volatility.
