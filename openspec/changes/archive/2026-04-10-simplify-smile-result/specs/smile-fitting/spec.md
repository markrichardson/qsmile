...
The system SHALL return a `SmileResult` plain dataclass (not generic) containing `model` (SmileModel instance with metadata and coordinate context), `residuals` (ndarray), `rmse` (float), and `success` (bool). The `model` field SHALL hold a fully coordinate-aware model instance. `SmileResult` SHALL NOT use `Generic` or `TypeVar`. The fitted model is NOT callable — use `result.model.evaluate(x)`.

#### Scenario: Access fitted model
- **WHEN** a user accesses `result.model` on a successful fit
...
- **WHEN** a user calls `result.model.evaluate(x)` with x values
...
- **WHEN** a user calls `result.model.transform(XCoord.FixedStrike, YCoord.Volatility).evaluate(strikes)`
...
- **THEN** `result.success` SHALL be `False` and `result.model` SHALL contain the best parameters found as a coordinate-aware model instance

### Requirement: fit() populates model with metadata and coordinates
The `fit()` function SHALL construct the fitted model instance with the `SmileMetadata` from the input `SmileData` and set `current_x_coord`/`current_y_coord` to the model's native coordinates. Type hints SHALL use `SmileModel` ABC directly (no TypeVar, no Generic).

#### Scenario: Fitted model carries metadata
- **WHEN** `fit(sd, SVIModel)` completes successfully
- **THEN** `result.model.metadata` SHALL be populated from the input `SmileData`'s metadata

#### Scenario: Fitted model in native coords
- **WHEN** `fit(sd, SVIModel)` completes
- **THEN** `result.model.current_x_coord` SHALL equal `SVIModel.native_x_coord`
- **AND** `result.model.current_y_coord` SHALL equal `SVIModel.native_y_coord`

#### Scenario: Fitted model can be transformed and evaluated
- **WHEN** `result.model.transform(XCoord.FixedStrike, YCoord.Volatility).evaluate(strikes)` is called after fitting
